...
3. **Docs de projeto** (`api/docs/`, `ui/docs/`): informações específicas de cada aplicação (como rodar, convenções locais).
...
4. Decisões arquiteturais relevantes → crie um ADR em `docs/adr/`.
5. O status da spec acompanha o ciclo: `Draft → Proposed → Approved → Implemented → Superseded`.
